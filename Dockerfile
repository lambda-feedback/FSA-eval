...
# the following is a fastapi workaround

FROM ghcr.io/lambda-feedback/evaluation-function-base/python:3.12 AS builder

RUN pip install poetry==1.8.3
# Install uvicorn (and fastapi if needed) BEFORE compile
...
RUN poetry run pip install fastapi
...
CMD ["python", "-m", "uvicorn", "evaluation_function.fastapi:app", "--host", "0.0.0.0", "--port", "8080"]
